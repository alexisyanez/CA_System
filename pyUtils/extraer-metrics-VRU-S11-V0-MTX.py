...
#for k in range(0,4):
	...
	for i in range(0,5): #(0,10):
		...
		PKT_Total_Send = []
		#List1 = [[],[]]
		name= Pathresults + namePrefix + Conf + DEN[l] + "-#" + str(i) + ".sca" #  "BL="+ Interval[k]+"," + Interval[k]+","+ Interval[k]+"," + Interval[k]  +"-#" + str(i) + ".sca" #DEN[l] + "BL="+ Interval[k] + END[l]  +"-#" + str(i) + ".sca" 
	...
	MeanRunsCBR2[l][k].append(np.mean(CBR2))  
	STDRunCBR2[l][k].append(np.std(CBR2))
