...
#include "modules/phy/NistErrorRate.h"
#include "veins/modules/utility/ConstsPhy.h"

using Veins::AirFrame;
using Veins::Radio;

simtime_t Decider80211p2::processNewSignal(AirFrame* msg) {
...
int Decider80211p2::getSignalState(AirFrame* frame) {
...
double Decider80211p2::calcChannelSenseRSSI(simtime_t_cref start, simtime_t_cref end) {
...
void Decider80211p2::calculateSinrAndSnrMapping(AirFrame* frame, Mapping **sinrMap, Mapping **snrMap) {
...
Mapping* Decider80211p2::calculateNoiseRSSIMapping(simtime_t_cref start, simtime_t_cref end, AirFrame *exclude) {
...
DeciderResult* Decider80211p2::checkIfSignalOk(AirFrame* frame) {
...
enum Decider80211p2::PACKET_OK_RESULT Decider80211p2::packetOk(double snirMin, double snrMin, int lengthMPDU, double bitrate) {
...
bool Decider80211p2::cca(simtime_t_cref time, AirFrame* exclude) {
...
simtime_t Decider80211p2::processSignalEnd(AirFrame* msg) {
...
double Decider80211p2::getCCAThreshold() {
	return 10 * log10(ccaThreshold);
}

void Decider80211p2::setCCAThreshold(double ccaThreshold_dBm) {
	ccaThreshold = pow(10, ccaThreshold_dBm / 10);
}

void Decider80211p2::switchToTx() {
	if (currentSignal.first != 0) {
		...
		if (allowTxDuringRx) {
			...
			throw cRuntimeError("Decider80211p2: mac layer requested phy to transmit a frame while currently receiving another");
		}
	}
}

void Decider80211p2::finish() {
