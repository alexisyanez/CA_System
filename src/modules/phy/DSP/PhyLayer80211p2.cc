...
void PhyLayer80211p2::initialize(int stage) {
	if (stage == 0) {
...
AnalogueModel* PhyLayer80211p2::getAnalogueModelFromName(std::string name, ParameterMap& params) {
...
AnalogueModel* PhyLayer80211p2::initializeLogNormalShadowing(ParameterMap& params) {
...
AnalogueModel* PhyLayer80211p2::initializeJakesFading(ParameterMap& params) {
...
AnalogueModel* PhyLayer80211p2::initializeBreakpointPathlossModel(ParameterMap& params) {
...
AnalogueModel* PhyLayer80211p2::initializeTwoRayInterferenceModel(ParameterMap& params) {
...
AnalogueModel* PhyLayer80211p2::initializeNakagamiFading(ParameterMap& params) {
...
AnalogueModel* PhyLayer80211p2::initializeSimplePathlossModel(ParameterMap& params){
...
AnalogueModel* PhyLayer80211p2::initializePERModel(ParameterMap& params) {
	double per = params["packetErrorRate"].doubleValue();
	return new PERModel(per);
}

Decider* PhyLayer80211p2::getDeciderFromName(std::string name, ParameterMap& params) {
...
AnalogueModel* PhyLayer80211p2::initializeSimpleObstacleShadowing(ParameterMap& params){
...
Decider* PhyLayer80211p2::initializeDecider80211p(ParameterMap& params) {
	double centerFreq = params["centerFrequency"];
	Decider80211p2* dec = new Decider80211p2(this, sensitivity, ccaThreshold, allowTxDuringRx, centerFreq, findHost()->getIndex(), collectCollisionStatistics, coreDebug);
	dec->setPath(getParentModule()->getFullPath());
	return dec;
}

void PhyLayer80211p2::changeListeningFrequency(double freq) {
	Decider80211p2* dec = dynamic_cast<Decider80211p2*>(decider);
	assert(dec);
	dec->changeFrequency(freq);
}
void PhyLayer80211p2::handleSelfMessage(cMessage* msg) {
...
AirFrame *PhyLayer80211p2::encapsMsg(cPacket *macPkt)
{
...
int PhyLayer80211p2::getRadioState() {
	return BasePhyLayer::getRadioState();
};

simtime_t PhyLayer80211p2::setRadioState(int rs) {
	if (rs == Radio::TX)
		decider->switchToTx();
	return BasePhyLayer::setRadioState(rs);
}

void PhyLayer80211p2::setCCAThreshold(double ccaThreshold_dBm) {
	ccaThreshold = pow(10, ccaThreshold_dBm / 10);
	((Decider80211p2 *)decider)->setCCAThreshold(ccaThreshold_dBm);
}
double PhyLayer80211p2::getCCAThreshold() {
...
void PhyLayer80211p2::turnOn2BT(bool state) {
