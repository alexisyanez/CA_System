...
#include "CycleAppLayer.h"

Define_Module(CycleAppLayer);

void CycleAppLayer::initialize(int stage) {
...
void CycleAppLayer::onBSM(BasicSafetyMessage* bsm) {
...
void CycleAppLayer::onWSM(WaveShortMessage* wsm) {
...
void CycleAppLayer::onWSA(WaveServiceAdvertisment* wsa) {
    //Your application has received a service advertisement from another car or RSU
    //code for handling the message goes here, see TraciDemo11p.cc for examples

}

void CycleAppLayer::handleSelfMsg(cMessage* msg) {
    switch (msg->getKind()) {
...
void CycleAppLayer::handlePositionUpdate(cObject* obj) {
...
bool CycleAppLayer::isNeighbor(std::list<std::pair<double,int>>mylist,int addressSearch){
...
std::list<std::pair<double,int>> CycleAppLayer::replace(std::list<std::pair<double,int>>mylist,int addressSearch, double UtxReplace){
...
double CycleAppLayer::calculateUtx(simtime_t CBR_r,double Dij_n, int Num_neig_n){
...
int* CycleAppLayer::makePriorList(std::list<std::pair<double,int>>mylist){
...
void CycleAppLayer::setingPLinWSM(int* list,WaveShortMessage* wsm){
...
int CycleAppLayer::getMyRank(WaveShortMessage* wsm, int my_id){
    unsigned int i;
