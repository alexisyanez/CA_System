...
#include "modules/mac/ieee80211p/DSP/Mac1609_4_2.h"
#include <iterator>
...
Define_Module(Mac1609_4_2);

void Mac1609_4_2::initialize(int stage) {
	BaseMacLayer::initialize(stage);
	if (stage == 0) {

		phy11p = FindModule<Mac80211pToPhy11pInterface_2*>::findSubModule(
		             getParentModule());
		assert(phy11p);
...
void Mac1609_4_2::handleSelfMsg(cMessage* msg) {
...
void Mac1609_4_2::handleUpperControl(cMessage* msg) {
	assert(false);
}

void Mac1609_4_2::handleUpperMsg(cMessage* msg) {
...
void Mac1609_4_2::handleLowerControl(cMessage* msg) {
	...
	else if (msg->getKind() == Mac80211pToPhy11pInterface_2::CHANNEL_BUSY) {
		channelBusy();
	}
	else if (msg->getKind() == Mac80211pToPhy11pInterface_2::CHANNEL_IDLE) {
		channelIdle();
	}
	else if (msg->getKind() == Decider80211p2::BITERROR || msg->getKind() == Decider80211p2::COLLISION) {
		statsSNIRLostPackets++;
		DBG_MAC << "A packet was not received due to biterrors" << std::endl;
	}
	else if (msg->getKind() == Decider80211p2::RECWHILESEND) {
	...
	if (msg->getKind() == Decider80211p2::COLLISION) {
...
void Mac1609_4_2::setActiveChannel(t_channel state) {
	activeChannel = state;
	assert(state == type_CCH || (useSCH && state == type_SCH));
}

void Mac1609_4_2::finish() {
	//clean up queues.
...
void Mac1609_4_2::attachSignal(Mac80211Pkt* mac, simtime_t startTime, double frequency, uint64_t datarate, double txPower_mW) {
...
Signal* Mac1609_4_2::createSignal(simtime_t start, simtime_t length, double power, uint64_t bitrate, double frequency) {
...
bool Mac1609_4_2::guardActive() const {
...
simtime_t Mac1609_4_2::timeLeftTillGuardOver() const {
	ASSERT(useSCH);
...
simtime_t Mac1609_4_2::timeLeftInSlot() const {
	ASSERT(useSCH);
...
void Mac1609_4_2::changeServiceChannel(int cN) {
	ASSERT(useSCH);
...
void Mac1609_4_2::setTxPower(double txPower_mW) {
	txPower = txPower_mW;
}
void Mac1609_4_2::setMCS(enum PHY_MCS mcs) {
	ASSERT2(mcs != MCS_DEFAULT, "invalid MCS selected");
	bitrate = getOfdmDatarate(mcs, BW_OFDM_10_MHZ);
	setParametersForBitrate(bitrate);
}

void Mac1609_4_2::setCCAThreshold(double ccaThreshold_dBm) {
	phy11p->setCCAThreshold(ccaThreshold_dBm);
}

void Mac1609_4_2::handleLowerMsg(cMessage* msg) {
	...
	DeciderResult80211_2 *macRes = dynamic_cast<DeciderResult80211_2*>(PhyToMacControlInfo::getDeciderResult(msg));
	ASSERT(macRes);
	DeciderResult80211_2 *res = new DeciderResult80211_2(*macRes);
...
int Mac1609_4_2::EDCA::queuePacket(t_access_category ac,WaveShortMessage* msg) {
...
void Mac1609_4_2::EDCA::createQueue(int aifsn, int cwMin, int cwMax,t_access_category ac) {
...
void Mac1609_4_2::EDCA::modifyQueue(int aifsn, int cwMin, int cwMax,t_access_category ac) { //método para cambiar la ventana de contención de la cola
...
Mac1609_4_2::t_access_category Mac1609_4_2::mapUserPriority(int prio) {
...
WaveShortMessage* Mac1609_4_2::EDCA::initiateTransmit(simtime_t lastIdle) {
...
simtime_t Mac1609_4_2::EDCA::startContent(simtime_t idleSince,bool guardActive) {
...
void Mac1609_4_2::EDCA::stopContent(bool allowBackoff, bool generateTxOp) {
	//update all Queues
...
void Mac1609_4_2::EDCA::backoff(t_access_category ac) {
...
void Mac1609_4_2::EDCA::postTransmit(t_access_category ac) {
...
void Mac1609_4_2::EDCA::revokeTxOPs() {
	for (std::map<t_access_category, EDCAQueue>::iterator iter = myQueues.begin(); iter != myQueues.end(); iter++) {
...
void Mac1609_4_2::channelBusySelf(bool generateTxOp) {
...
void Mac1609_4_2::channelIdle(bool afterSwitch) {
...
void Mac1609_4_2::setParametersForBitrate(uint64_t bitrate) {
...
bool Mac1609_4_2::isChannelSwitchingActive() {
    return useSCH;
}

simtime_t Mac1609_4_2::getSwitchingInterval() {
    return SWITCHING_INTERVAL_11P;
}

simtime_t Mac1609_4_2::getBusyTime() {
    return statsTotalBusyTime;
}

int Mac1609_4_2::getCWcur() {
    return MyCWcur;
}

void Mac1609_4_2::setCWcur(int CW_1) {
    MyCWcur = CW_1;
}

long Mac1609_4_2::getMyCollisions() {
    return MyColl;
}

long Mac1609_4_2::getNTIB() {
    return statsNumBackoff;
}

long Mac1609_4_2::getNBR() {
    return statsReceivedBroadcasts;
}

bool Mac1609_4_2::isCurrentChannelCCH() {
    return (activeChannel ==  type_CCH);
}

simtime_t Mac1609_4_2::getFrameDuration(int payloadLengthBits, enum PHY_MCS mcs) const {
    simtime_t duration;
    if (mcs == MCS_DEFAULT) {
